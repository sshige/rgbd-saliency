...
__global__ void CalculateRegionFeatures(const int nthreads,
    ...
    const Dtype* const slic_index_data, const float* const query_indexes,
					Handcrafted* query_features, Dtype* label_output, Dtype* query_output) {
  CUDA_KERNEL_LOOP(c_index, nthreads) {
    ...
    // copy index in order to synchronize depth current and mcs current
    query_output[index] = query_indexes[index];
    ...
    for (int y = y_start; y < y_end; y++) {
      for (int x = x_start; x < x_end; x++) {
        if (slic_index_data[(n * height + y) * width + x] == query_sp_idx) {
	  //printf("slic_index_data : %f \n", slic_index_data[(n * height + y) * width + x]);
	  // printf("(n * height + y) * width + x : %d \n",
	  // 	 (n * height + y) * width + x);
	  //printf("query_sp_idx : %d \n", query_sp_idx);
	  // printf("data_slice : %f \n", data_slice[y * width + x]);
          count += 1;
...
void LowlevelDistanceLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype* query_output = NULL;
  if (top.size() > 1) {
    label_output = top[1]->mutable_gpu_data();
    //printf("clear1\n");
    query_output = top[2]->mutable_gpu_data();
  }
  //printf("clear2\n");    
  ...
  CalculateRegionFeatures<<<CAFFE_GET_BLOCKS(block_cnt),
    CAFFE_CUDA_NUM_THREADS>>>(block_cnt, N_, mcs_channels, H_, W_, R_,
        slic_xdim, slic_ydim, sp_size_, mcs_gpu_ptr, labelmap_ptr,
			      slic_idx_ptr, query_idx_ptr, query_fptr, label_output, query_output);
